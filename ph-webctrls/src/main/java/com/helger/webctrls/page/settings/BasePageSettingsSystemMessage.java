...
public class BasePageSettingsSystemMessage <WPECTYPE extends IWebPageExecutionContext> extends AbstractWebPageExt <WPECTYPE>
{
  ...
  {
    return SystemMessageUIHelper.createDefaultBox (aWPEC);
  }
  ...
  public void fillContent (@Nonnull final WPECTYPE aWPEC)
  {
        ...
        if (checkCSRFNonce (aWPEC).isContinue ())
        {
          ...
          if (eChange.isChanged ())
          {
            aNodeList.addChild (getStyler ().createSuccessBox (aWPEC,
                                                               EText.SAVE_SUCCESS.getDisplayText (aDisplayLocale)));
          }
        }
      }
      else
    ...
    if (bShowList)
    {
      ...
      {
        // No message present
        aForm.addChild (getStyler ().createInfoBox (aWPEC, EText.NO_SYSTEM_MESSAGE.getDisplayText (aDisplayLocale)));
      }
