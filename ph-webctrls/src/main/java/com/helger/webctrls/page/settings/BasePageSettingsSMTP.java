...
import com.helger.webctrls.page.ui.HCCharsetSelect;

public class BasePageSettingsSMTP <WPECTYPE extends IWebPageExecutionContext> extends AbstractWebPageForm <NamedSMTPSettings, WPECTYPE>
{
  @Translatable
  ...
  {
    final Locale aDisplayLocale = aWPEC.getDisplayLocale ();
    aForm.addChild (getStyler ().createQuestionBox (aWPEC,
    ...
    if (aWPEC.hasAction (ACTION_TEST_MAIL) && aSelectedObject != null)
    {
      ...
      if (aWPEC.hasSubAction (ACTION_PERFORM))
      {
        ...
        if (aFormErrors.isEmpty ())
        {
          ...
          aNodeList.addChild (getStyler ().createSuccessBox (aWPEC,
                                                             EText.SUCCESS_TEST_MAIL.getDisplayText (aDisplayLocale)));
