...
public class BasePageMonitoringAjaxFunctions <WPECTYPE extends IWebPageExecutionContext> extends AbstractWebPageExt <WPECTYPE>
{
  ...
  public BasePageMonitoringAjaxFunctions (@Nonnull @Nonempty final String sID)
  {
    super (sID, EWebPageText.PAGE_NAME_MONITORING_AJAX_FUNCTIONS.getAsMLT ());
  }

  public BasePageMonitoringAjaxFunctions (@Nonnull @Nonempty final String sID, @Nonnull final String sName)
  {
    super (sID, sName);
  }
  ...
  protected void fillContent (@Nonnull final WPECTYPE aWPEC)
  {
    ...
    for (final IApplicationScope aAppScope : WebScopeManager.getGlobalScope ().getAllApplicationScopes ().values ())
    {
      ...
      if (aMgr != null)
      {
        ...
        // Show all registered AJAX functions
        {
          final IHCTable <?> aTable = getStyler ().createTable (HCCol.star (), HCCol.star (), HCCol.star ())
        ...
        // Show all callbacks
        {
          ...
          final IHCTable <?> aTable = getStyler ().createTable (HCCol.star (), HCCol.star ()).setID (getID () +
                                                                                                     sAppScopeID +
                                                                                                     "-cb");
    ...
    if (aTabBox.getTabCount () > 0)
      ...
      aNodeList.addChild (getStyler ().createInfoBox (aWPEC, EText.MSG_NONE_FOUND.getDisplayText (aDisplayLocale)));
  }
}
