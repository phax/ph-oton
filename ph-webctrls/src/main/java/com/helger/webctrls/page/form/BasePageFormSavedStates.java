 ...
 */
package com.helger.webctrls.page.form;
 ...
 */
public class BasePageFormSavedStates <WPECTYPE extends IWebPageExecutionContext> extends AbstractWebPageForm <FormState, WPECTYPE>
{
  @Translatable
      ...
      if (aWPEC.hasSubAction (CHCParam.ACTION_SAVE))
      {
        if (FormStateManager.getInstance ().deleteAllFormStates ().isChanged ())
          aNodeList.addChild (getStyler ().createSuccessBox (aWPEC,
                                                             EText.DELETE_ALL_SUCCESS.getDisplayText (aDisplayLocale)));
        else
          aNodeList.addChild (getStyler ().createErrorBox (aWPEC,
                                                           EText.DELETE_ALL_ERROR.getDisplayText (aDisplayLocale)));
        return true;
      }
