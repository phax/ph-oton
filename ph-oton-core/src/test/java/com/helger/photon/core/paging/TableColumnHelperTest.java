 ...
 */
public final class TableColumnHelperTest
{
  /**
  ...
  @Test
  public void testColumnFlags ()
  {
    assertTrue (COL_ALL.isSortable ());
    assertTrue (COL_ALL.isSearchable ());
    assertTrue (COL_ALL.isDefaultSortColumn ());

    assertTrue (COL_LENGTH.isSortable ());
    assertFalse (COL_LENGTH.isSearchable ());
    assertFalse (COL_LENGTH.isDefaultSortColumn ());

    assertFalse (COL_FIRST.isSortable ());
    assertTrue (COL_FIRST.isSearchable ());

    assertEquals ("b", COL_FIRST.getSearchValue ("bbb"));
    assertNull (COL_LENGTH.getSearchValue ("bbb"));
    assertTrue (COL_ALL.matchesSearchText ("bbb", "BB"));
    assertFalse (COL_LENGTH.matchesSearchText ("bbb", "3"));
  }
  ...
  public void testSearchPredicate ()
  {
    ...
    assertNull (TableColumnHelper.getSearchPredicate (COLUMNS, ""));
    assertNull (TableColumnHelper.getSearchPredicate (COLUMNS, (String []) null));
    assertNull (TableColumnHelper.getSearchPredicate (COLUMNS, new String [0]));

    // Case insensitive "contains" on any searchable column
    Predicate <String> aFilter = TableColumnHelper.getSearchPredicate (COLUMNS, "BB");
  ...
  public void testGetCount ()
  {
    ...
    assertEquals (1, TableColumnHelper.getCount (COLUMNS, _list (), "c"));
    assertEquals (0, TableColumnHelper.getCount (COLUMNS, _list (), "x"));
  }
}
