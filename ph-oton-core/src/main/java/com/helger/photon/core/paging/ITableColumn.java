 ...
 * the single source of truth that ties the column shown in the UI to the way the underlying objects
 * are sorted and searched, so that these cannot drift apart. It is usually implemented as an enum
 * per domain object.<br>
 ...
 * refer to - see {@link TableColumnHelper}.<br>
 * An application that stores its domain objects in a database usually derives an own interface from
 * this one, adding the mapping onto the columns respectively fields of the underlying data store.
 *
 ...
 * @since 10.6.0
 */
public interface ITableColumn <DATATYPE> extends IHasID <String>
{
  /**
   ...
   *         otherwise. The default implementation covers a column, if it has a search value
   *         provider.
   */
   ...
   *         otherwise. The default implementation allows sorting, if a comparator is present.
   */
   ...
   * with a non-<code>null</code> value form the default order together, in their declaration order,
   * which is how a composite key like "service group, then document type" is expressed.<br>
