...
function(a,b){c.push(f[b])});return c}return f[a]}function c(a){if(b.isArray(a)){var c=[];b.each(a,function(a,b){c.push(f[g[b]])});return c}return f[g[a]]}function e(a){var c=[];a=[].concat(a);b.each(a,function(a,d){var g=h[d];void 0!==g&&b.each(g,function(a,b){c.push(f[b])})});return c}var f=[],g={},h={},k={},l={};b.extend(this,{addFile:function(c){var d=c.status||b.status.SUBMITTING,e=f.push({name:c.name,originalName:c.name,uuid:c.uuid,size:null==c.size?-1:c.size,status:d})-1;c.batchId&&(f[e].batchId=
...
[];null==k._getFileState(a).loaded&&(k._getFileState(a).loaded=0);r&&!1===l.onResume(a,d,g)&&(n.reset(a),e=n.nextPart(a),g=k._getChunkData(a,e),r=!1);null==e&&0===q.length?n.finalize(a):(h(b.format("Sending chunked upload request for item {}.{}, bytes {}-{} of {}.",a,e,g.start+1,g.end,c)),l.onUploadChunk(a,d,k._getChunkDataForCallback(g)),q.push(e),k._getFileState(a).chunking.inProgress=q,f&&m.open(a,e),f&&m.available()&&k._getFileState(a).chunking.remaining.length&&n.sendNext(a),k.uploadChunk(a,
e,r).then(function(c,d){h("Chunked upload request succeeded for "+a+", chunk "+e);k.clearCachedChunk(a,e);var f=k._getFileState(a).chunking.inProgress||[],g=p.normalizeResponse(c,!0),x=b.indexOf(f,e);h(b.format("Chunk {} for file {} uploaded successfully.",e,a));n.done(a,e,g,d);0<=x&&f.splice(x,1);k._maybePersistChunkedState(a);n.hasMoreParts(a)||0!==f.length?n.hasMoreParts(a)?n.sendNext(a):h(b.format("File ID {} has no more chunks to send and these chunk indexes are still marked as in-progress: {}",
a,JSON.stringify(f))):n.finalize(a)},function(c,g){h("Chunked upload request failed for "+a+", chunk "+e);k.clearCachedChunk(a,e);var x=p.normalizeResponse(c,!1),r;x.reset?n.reset(a):(r=b.indexOf(k._getFileState(a).chunking.inProgress,e),0<=r&&(k._getFileState(a).chunking.inProgress.splice(r,1),k._getFileState(a).chunking.remaining.unshift(e)));k._getFileState(a).temp.ignoreFailure||(f&&(k._getFileState(a).temp.ignoreFailure=!0,h(b.format("Going to attempt to abort these chunks: {}. These are currently in-progress: {}.",
JSON.stringify(Object.keys(k._getXhrs(a))),JSON.stringify(k._getFileState(a).chunking.inProgress))),b.each(k._getXhrs(a),function(c,d){h(b.format("Attempting to abort file {}.{}. XHR readyState {}. ",a,c,d.readyState));d.abort();d._cancelled=!0}),k.moveInProgressToRemaining(a),m.free(a,!0)),l.onAutoRetry(a,d,x,g)||p.cleanup(a,x,g))}).done(function(){k.clearXhr(a,e)}))}},m={_open:[],_openChunks:{},_waiting:[],available:function(){var a=l.maxConnections,c=0,d=0;b.each(m._openChunks,function(a,b){c++;
d+=b.length});return a-(m._open.length-c+d)},free:function(a,c){var d=!c,e=b.indexOf(m._waiting,a),f=b.indexOf(m._open,a);delete m._openChunks[a];p.getProxyOrBlob(a)instanceof b.BlobProxy&&(h("Generated blob upload has ended for "+a+", disposing generated blob."),delete k._getFileState(a).file);0<=e?m._waiting.splice(e,1):d&&0<=f&&(m._open.splice(f,1),d=m._waiting.shift(),0<=d&&(m._open.push(d),p.start(d)))},getWaitingOrConnected:function(){var a=[];b.each(m._openChunks,function(b,c){c&&c.length&&
a.push(parseInt(b))});b.each(m._open,function(b,c){m._openChunks[c]||a.push(parseInt(c))});return a=a.concat(m._waiting)},isUsingConnection:function(a){return 0<=b.indexOf(m._open,a)},open:function(a,b){null==b&&m._waiting.push(a);if(m.available()){if(null==b)m._waiting.pop(),m._open.push(a);else{var c=m._openChunks[a]||[];c.push(b);m._openChunks[a]=c}return!0}return!1},reset:function(){m._waiting=[];m._open=[]}},q={send:function(a,b){k._getFileState(a).loaded=0;h("Sending simple upload request for "+
a);k.uploadFile(a).then(function(c,d){h("Simple upload request succeeded for "+a);var e=p.normalizeResponse(c,!0),f=l.getSize(a);l.onProgress(a,b,f,f);p.maybeNewUuid(a,e);p.cleanup(a,e,d)},function(c,d){h("Simple upload request failed for "+a);var e=p.normalizeResponse(c,!1);l.onAutoRetry(a,b,e,d)||p.cleanup(a,e,d)})}},p={cancel:function(a){h("Cancelling "+a);l.paramsStore.remove(a);m.free(a)},cleanup:function(a,b,c){var d=l.getName(a);l.onComplete(a,d,b,c);k._getFileState(a)&&k._clearXhrs&&k._clearXhrs(a);
m.free(a)},getProxyOrBlob:function(a){return k.getProxy&&k.getProxy(a)||k.getFile&&k.getFile(a)},initHandler:function(){k=new (d?b[d]:b.traditional)[(b.supportedFeatures.ajaxUploading?"Xhr":"Form")+"UploadHandler"](l,{getDataByUuid:l.getDataByUuid,getName:l.getName,getSize:l.getSize,getUuid:l.getUuid,log:h,onCancel:l.onCancel,onProgress:l.onProgress,onUuidChanged:l.onUuidChanged});k._removeExpiredChunkingRecords&&k._removeExpiredChunkingRecords()},isDeferredEligibleForUpload:function(a){return l.isQueued(a)},
maybeDefer:function(a,c){if(c&&!k.getFile(a)&&c instanceof b.BlobProxy)l.onUploadPrep(a),h("Attempting to generate a blob on-demand for "+a),c.create().then(function(b){h("Generated an on-demand blob for "+a);k.updateBlob(a,b);l.setSize(a,b.size);k.reevaluateChunking(a);p.maybeSendDeferredFiles(a)},function(c){var d={};c&&(d.error=c);h(b.format("Failed to generate blob for ID {}.  Error message: {}.",a,c),"error");l.onComplete(a,l.getName(a),b.extend(d,g),null);p.maybeSendDeferredFiles(a);m.free(a)});
...
!c&&(d.error=a));d.success=c;return d},now:function(a){var d=l.getName(a);if(!c.isValid(a))throw new b.Error(a+" is not a valid file ID to upload!");l.onUpload(a,d);e&&k._shouldChunkThisFile(a)?n.sendNext(a):q.send(a,d)},start:function(a){var b=p.getProxyOrBlob(a);if(b)return p.maybeDefer(a,b);p.now(a);return!0}};b.extend(this,{add:function(a,b){k.add.apply(this,arguments)},upload:function(a){return m.open(a)?p.start(a):!1},retry:function(a){f&&(k._getFileState(a).temp.ignoreFailure=!1);return m.isUsingConnection(a)?
p.start(a):c.upload(a)},cancel:function(a){var c=k.cancel(a);b.isGenericPromise(c)?c.then(function(){p.cancel(a)}):!1!==c&&p.cancel(a)},cancelAll:function(){var a=m.getWaitingOrConnected(),b;if(a.length)for(b=a.length-1;0<=b;b--)c.cancel(a[b]);m.reset()},getFile:function(a){return k.getProxy&&k.getProxy(a)?k.getProxy(a).referenceBlob:k.getFile&&k.getFile(a)},isProxied:function(a){return!(!k.getProxy||!k.getProxy(a))},getInput:function(a){if(k.getInput)return k.getInput(a)},reset:function(){h("Resetting upload handler");
c.cancelAll();m.reset();k.reset()},expunge:function(a){if(c.isValid(a))return k.expunge(a)},isValid:function(a){return k.isValid(a)},getResumableFilesData:function(){return k.getResumableFilesData?k.getResumableFilesData():[]},getThirdPartyFileId:function(a){if(c.isValid(a))return k.getThirdPartyFileId(a)},pause:function(a){return c.isResumable(a)&&k.pause&&c.isValid(a)&&k.pause(a)?(m.free(a),k.moveInProgressToRemaining(a),!0):!1},isResumable:function(a){return!!k.isResumable&&k.isResumable(a)}});
...
(a=d[a])&&a()}})};b.FormUploadHandler=function(a){function d(a,c){var d=a.id,f=d.split("_")[0],g=q(f);h[g]=c;k[f]=b(a).attach("load",function(){e.getInput(f)&&(p("Received iframe load event for CORS upload request (iframe name "+d+")"),l[d]=setTimeout(function(){var a="No valid message received from loaded iframe for iframe name "+d;p(a,"error");c({error:a})},1E3))});t.receiveMessage(d,function(a){p("Received the following window message: '"+a+"'");d.split("_");var b=e._parseJsonResponse(a),c=b.uuid;
c&&h[c]?(p("Handling response for iframe name "+d),clearTimeout(l[d]),delete l[d],e._detachLoadEvent(d),a=h[c],delete h[c],t.stopReceivingMessages(d),a(b)):c||p("'"+a+"' does not contain a UUID - ignoring.")})}var c=a.options,e=this,f=a.proxy,g=b.getUniqueId(),h={},k={},l={},n=c.isCors,m=c.inputName,q=f.getUuid,p=f.log,t=new b.WindowReceiveMessage({log:p});b.extend(this,new b.UploadHandler(a));b.override(this,function(a){return{add:function(c,d){a.add(c,{input:d});d.setAttribute("name",m);d.parentNode&&
b(d).remove()},expunge:function(c){delete k[c];n&&(clearTimeout(l[c]),delete l[c],t.stopReceivingMessages(c));var d=document.getElementById(e._getIframeName(c));d&&(d.setAttribute("src","javascript:false;"),b(d).remove());a.expunge(c)},isValid:function(b){return a.isValid(b)&&void 0!==e._getFileState(b).input}}});b.extend(this,{getInput:function(a){return e._getFileState(a).input},_attachLoadEvent:function(a,c){var e;n?d(a,c):k[a.id]=b(a).attach("load",function(){p("Received response for "+a.id);
...
delete k[a])},_getIframeName:function(a){return a+"_"+g},_initFormForUpload:function(a){var c=a.endpoint,d=a.params,e=a.paramsInBody,f=a.targetName;a=b.toElement("<form method='"+a.method+"' enctype='multipart/form-data'></form>");var g=c;e?b.obj2Inputs(d,a):g=b.obj2url(d,c);a.setAttribute("action",g);a.setAttribute("target",f);a.style.display="none";document.body.appendChild(a);return a},_parseJsonResponse:function(a){var c={};try{c=b.parseJson(a)}catch(d){p("Error when attempting to parse iframe upload response ("+
d.message+")","error")}return c}})};b.XhrUploadHandler=function(a){function d(a){b.each(c._getXhrs(a),function(b,d){var e=c._getAjaxRequester(a,b);d.onreadystatechange=null;d.upload.onprogress=null;d.abort();e&&e.canceled&&e.canceled(a)})}var c=this,e=a.options.namespace,f=a.proxy,g=a.options.chunking,h=a.options.resume,k=g&&a.options.chunking.enabled&&b.supportedFeatures.chunking,l=h&&a.options.resume.enabled&&k&&b.supportedFeatures.resume,n=f.getName,m=f.getSize,q=f.getUuid,p=f.getEndpoint,t=f.getDataByUuid,
...
c.log("Submitting delete file request for "+a);"DELETE"===c.method?d.initTransport(a).withPath(b).withParams(g).send():(g[c.uuidParamName]=b,d.initTransport(a).withParams(g).send())}})};(function(){function a(a,c,e,f){var n=document.createElement("canvas"),m=e.mime||"image/jpeg",q=new b.Promise;d(a,c,n,e,f).then(function(){q.success(n.toDataURL(m,e.quality||.8))});return q}function d(a,d,f,l,n){var m=a.naturalWidth,q=a.naturalHeight,p=l.width,t=l.height,w=f.getContext("2d"),u=new b.Promise;w.save();
if(l.resize)return c({blob:d,canvas:f,image:a,imageHeight:q,imageWidth:m,orientation:l.orientation,resize:l.resize,targetHeight:t,targetWidth:p});if(!b.supportedFeatures.unlimitedScaledImageSize){d=p;var v=t;if(!b.ios())throw new b.Error("Downsampled dimensions can only be reliably calculated for iOS!");d=5241E3<v*d?{newHeight:Math.round(Math.sqrt(v/d*5241E3)),newWidth:Math.round(Math.sqrt(d/v*5241E3))}:void 0;d&&(b.log(b.format("Had to reduce dimensions due to device limitations from {}w / {}h to {}w / {}h",
p,t,d.newWidth,d.newHeight),"warn"),p=d.newWidth,t=d.newHeight)}e(f,p,t,l.orientation);if(b.ios()){l=a.naturalWidth;d=a.naturalHeight;v=document.createElement("canvas");1048576<l*d?(v.width=v.height=1,d=v.getContext("2d"),d.drawImage(a,-l+1,0),l=0===d.getImageData(0,0,1,1).data[3]):l=!1;l&&(m/=2,q/=2);l=document.createElement("canvas");if(n){n=q;var x=document.createElement("canvas");d=0;var r=v=n,A;x.width=1;x.height=n;x=x.getContext("2d");x.drawImage(a,0,0);for(x=x.getImageData(0,0,1,n).data;r>
d;)A=x[4*(r-1)+3],0===A?v=r:d=r,r=v+d>>1;n=r/n;n=0===n?1:n}else n=1;p=Math.ceil(1024*p/m);t=Math.ceil(1024*t/q/n);d=n=0;l.width=l.height=1024;for(v=l.getContext("2d");n<q;){for(x=r=0;r<m;)v.clearRect(0,0,1024,1024),v.drawImage(a,-r,-n),w.drawImage(l,0,0,1024,1024,x,d,p,t),r+=1024,x+=p;n+=1024;d+=t}w.restore()}else w.drawImage(a,0,0,p,t);f.qqImageRendered&&f.qqImageRendered();u.success();return u}function c(a){var c=a.blob,d=a.image,f=a.imageHeight,n=a.imageWidth,m=a.orientation,q=new b.Promise,p=
a.resize,t=document.createElement("canvas"),w=t.getContext("2d"),u=a.canvas,v=a.targetHeight;a=a.targetWidth;e(t,n,f,m);u.height=v;u.width=a;w.drawImage(d,0,0);p({blob:c,height:v,image:d,sourceCanvas:t,targetCanvas:u,width:a}).then(function(){u.qqImageRendered&&u.qqImageRendered();q.success()},q.failure);return q}function e(a,b,c,d){switch(d){case 5:case 6:case 7:case 8:a.width=c;a.height=b;break;default:a.width=b,a.height=c}a=a.getContext("2d");switch(d){case 2:a.translate(b,0);a.scale(-1,1);break;
...
e.maxHeight,w=!this.blob||"image/jpeg"===this.blob.type,u=c.tagName.toLowerCase(),v;if(this.imageLoadListeners)this.imageLoadListeners.push(function(){f.render(c,e)});else if(m&&!q?q=n*m/l<<0:q&&!m?m=l*q/n<<0:(m=l,q=n),p&&m>p&&(m=p,q=n*m/l<<0),t&&q>t&&(q=t,m=l*q/n<<0),v={width:m,height:q},b.each(e,function(a,b){v[a]=b}),"img"===u?function(){var b=c.src;a(f.srcImage,f.blob,v,w).then(function(a){c.src=a;b===c.src&&c.onload()})}():"canvas"===u&&d(this.srcImage,this.blob,c,v,w),"function"===typeof this.onrender)this.onrender(c)};
...
d+18,2).then(function(a){if(e)return f.success(c(a));f.success(parseInt(a,16))});return f}function k(a,d){var e=b.extend([],l),f={};b.each(d,function(b,d){var g=d.slice(0,4),h=a?c(g):parseInt(g,16),g=e.indexOf(h),k;0<=g&&(k=n[h].name,h=n[h].bytes,h=d.slice(16,16+2*h),f[k]=a?c(h):parseInt(h,16),e.splice(g,1));if(0===e.length)return!1});return f}var l=[274],n={274:{name:"Orientation",bytes:2}};b.extend(this,{parse:function(){var c=new b.Promise,e=function(a){d(b.format("EXIF header parse failed: '{}' ",
...
{queryServer:function(){var a=b.extend({},c.params);c.log("Session query request.");d.initTransport("sessionRefresh").withParams(a).withCacheBuster().send()}})};b.Scaler=function(a,d){var c=a.customResizer,e=a.sendOriginal,f=a.orient,g=a.defaultType,h=a.defaultQuality/100,k=a.failureText,l=a.includeExif,n=this._getSortedSizes(a.sizes);b.extend(this,{enabled:b.supportedFeatures.scaling&&0<n.length,getFileRecords:function(a,q,p){var t=this,w=[],u=p.blob?p.blob:p;(new b.Identify(u,d)).isPreviewableSync()?
(b.each(n,function(a,e){var m=t._determineOutputType({defaultType:g,requestedType:e.type,refType:u.type});w.push({uuid:b.getUniqueId(),name:t._getName(q,{name:e.name,type:m,refType:u.type}),blob:new b.BlobProxy(u,b.bind(t._generateScaledImage,t,{customResizeFunction:c,maxSize:e.maxSize,orient:f,type:m,quality:h,failedText:k,includeExif:l,log:d}))})}),w.push({uuid:a,name:q,size:u.size,blob:e?u:null})):w.push({uuid:a,name:q,size:u.size,blob:u});return w},handleNewFile:function(a,c,d,e,f,g,h,k){var l=
[],n=null,E=k.addFileToHandler,y=k.uploadData,B=k.paramsStore,F=b.getUniqueId();b.each(this.getFileRecords(d,c,a),function(a,c){var d=c.size;c.blob instanceof b.BlobProxy&&(d=-1);d=y.addFile({uuid:c.uuid,name:c.name,size:d,batchId:g,proxyGroupId:F});c.blob instanceof b.BlobProxy?l.push(d):n=d;c.blob?(E(d,c.blob),f.push({id:d,file:c.blob})):y.setStatus(d,b.status.REJECTED)});null!==n&&(b.each(l,function(a,b){var c={qqparentuuid:y.retrieve({id:n}).uuid,qqparentsize:y.retrieve({id:n}).size};c[h]=y.retrieve({id:b}).uuid;
y.setParentId(b,n);B.addReadOnly(b,c)}),l.length&&function(){var a={};a[h]=y.retrieve({id:n}).uuid;B.addReadOnly(n,a)}())}})};b.extend(b.Scaler.prototype,{scaleImage:function(a,d,c){if(!b.supportedFeatures.scaling)throw new b.Error("Scaling is not supported in this browser!");var e=new b.Promise,f=c.log,g=c.getFile(a),h=(c=c.uploadData.retrieve({id:a}))&&c.name,k=c&&c.uuid,l=new b.Scaler({customResizer:d.customResizer,sendOriginal:!1,orient:d.orient,defaultType:d.type||null,defaultQuality:d.quality,
...
a);return a.sort(function(a,b){return a.maxSize>b.maxSize?1:a.maxSize<b.maxSize?-1:0})},_generateScaledImage:function(a,d){var c=this,e=a.customResizeFunction,f=a.log,g=a.maxSize,h=a.orient,k=a.type,l=a.quality,n=a.failedText,m=a.includeExif&&"image/jpeg"===d.type&&"image/jpeg"===k,q=new b.Promise,p=new b.ImageGenerator(f),t=document.createElement("canvas");f("Attempting to generate scaled version for "+d.name);p.generate(d,t,{maxSize:g,orient:h,customResizeFunction:e}).then(function(){var a=t.toDataURL(k,
l),e=function(){f("Success generating scaled version for "+d.name);var c=b.dataUriToBlob(a);q.success(c)};m?c._insertExifHeader(d,a,f).then(function(b){a=b;e()},function(){f("Problem inserting EXIF header into scaled image.  Using scaled image w/out EXIF data.","error");e()}):e()},function(){f("Failed attempt to generate scaled version for "+d.name,"error");q.failure(n)});return q},_insertExifHeader:function(a,d,c){var e=new FileReader,f=new b.Promise,g="";e.onload=function(){g=e.result;f.success(b.ExifRestorer.restore(g,
...
n=function(a){var b=d(a);0<b&&(m(a,0,b),c[a]={loaded:0,total:b})},m=function(a,b,d){var g=c[a]?c[a].loaded:0;a=c[a]?c[a].total:0;-1===b&&-1===d?(e-=g,f-=a):(b&&(e+=b-g),d&&(f+=d-a));k(e,f)};b.extend(this,{onAllComplete:function(a,b,c){if(0===b.length||l(b,c))k(f,f),this.reset()},onStatusChange:function(a,d,e){e===b.status.CANCELED||e===b.status.REJECTED?(m(a,-1,-1),delete c[a]):e===b.status.SUBMITTING&&n(a)},onIndividualProgress:function(a,b,d){m(a,b,d);c[a]={loaded:b,total:d}},onNewSize:function(a){n(a)},
...
t=function(c,d,h,l){var m=new FormData,n=a.method,p=a.endpointStore.get(l),q=e(l),t=f(l);c[a.uuidName]=g(l);c[a.filenameParam]=q;k&&(c[a.totalFileSizeName]=t);a.paramsInBody||(k||(c[a.inputName]=q),p=b.obj2url(c,p));d.open(n,p,!0);a.cors.expected&&a.cors.sendCredentials&&(d.withCredentials=!0);return k?(a.paramsInBody&&b.obj2FormData(c,m),m.append(a.inputName,h),m):h},w=function(d,e){var f=a.customHeaders.get(d),g=c.getFile(d);e.setRequestHeader("Accept","application/json");e.setRequestHeader("X-Requested-With",
