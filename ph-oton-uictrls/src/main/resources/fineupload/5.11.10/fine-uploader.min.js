...
return("draggable"in a||"ondragstart"in a&&"ondrop"in a)&&!b.android()&&!b.ios()}var d,c,e,f,g,h,k,l,m,n,p,q,r,y,v;c=(f=(d=function(){var a=!0,c;try{c=document.createElement("input"),c.type="file",b(c).hide(),c.disabled&&(a=!1)}catch(d){a=!1}return a}())&&b.isXhrUploadSupported())&&!b.androidStock();g=(e=f&&a())&&(b.chrome()||b.opera())&&void 0!==navigator.userAgent.match(/Chrome\/[2][1-9]|Chrome\/[3-9][0-9]/);h=f&&b.isFileChunkingSupported();if(k=f&&h)try{k=!!window.localStorage&&b.isFunction(window.localStorage.setItem)}catch(w){k=
!1}l=f&&(b.chrome()||b.opera())&&void 0!==navigator.userAgent.match(/Chrome\/[1][4-9]|Chrome\/[2-9][0-9]/);m=d&&(void 0!==window.postMessage||f);p=window.XMLHttpRequest?void 0!==b.createXhrInstance().withCredentials:!1;n=void 0!==window.XDomainRequest;q=window.XMLHttpRequest&&void 0!==b.createXhrInstance().withCredentials?!0:void 0!==window.XDomainRequest;r=void 0!==document.createElement("input").webkitdirectory;y=f&&void 0!==window.FileReader;v=f?!b.androidStock()&&!b.iosChrome():!1;return{ajaxUploading:f,
blobUploading:c,canDetermineSize:f,chunking:h,deleteFileCors:q,deleteFileCorsXdr:n,deleteFileCorsXhr:p,dialogElement:!!window.HTMLDialogElement,fileDrop:e,folderDrop:g,folderSelection:r,imagePreviews:y,imageValidation:y,itemSizeValidation:f,pause:h,progressBar:v,resume:k,scaling:y&&c,tiffPreviews:b.safari(),unlimitedScaledImageSize:!b.ios(),uploading:d,uploadCors:m,uploadCustomHeaders:f,uploadNonMultipart:f,uploadViaPaste:l}}();b.isGenericPromise=function(a){return!!(a&&a.then&&b.isFunction(a.then))};
...
c=l.getName(a);h("All chunks have been uploaded for "+a+" - finalizing....");k.finalizeChunks(a).then(function(d,e){h("Finalize successful for "+a);var f=q.normalizeResponse(d,!0);l.onProgress(a,c,b,b);k._maybeDeletePersistedChunkData(a);q.cleanup(a,f,e)},function(b,d){var e=q.normalizeResponse(b,!1);h("Problem finalizing chunks for file ID "+a+" - "+e.error,"error");e.reset&&m.reset(a);l.onAutoRetry(a,c,e,d)||q.cleanup(a,e,d)})},handleFailure:function(a,c,d,e){var g=l.getName(c);h("Chunked upload request failed for "+
c+", chunk "+a);k.clearCachedChunk(c,a);d=q.normalizeResponse(d,!1);var p;d.reset?m.reset(c):(p=b.indexOf(k._getFileState(c).chunking.inProgress,a),0<=p&&(k._getFileState(c).chunking.inProgress.splice(p,1),k._getFileState(c).chunking.remaining.unshift(a)));k._getFileState(c).temp.ignoreFailure||(f&&(k._getFileState(c).temp.ignoreFailure=!0,h(b.format("Going to attempt to abort these chunks: {}. These are currently in-progress: {}.",JSON.stringify(Object.keys(k._getXhrs(c))),JSON.stringify(k._getFileState(c).chunking.inProgress))),
b.each(k._getXhrs(c),function(a,d){h(b.format("Attempting to abort file {}.{}. XHR readyState {}. ",c,a,d.readyState));d.abort();d._cancelled=!0}),k.moveInProgressToRemaining(c),n.free(c,!0)),l.onAutoRetry(c,g,d,e)||q.cleanup(c,d,e))},hasMoreParts:function(a){return!!k._getFileState(a).chunking.remaining.length},nextPart:function(a){var b=k._getFileState(a).chunking.remaining.shift();b>=k._getTotalChunks(a)&&(b=null);return b},reset:function(a){h("Server or callback has ordered chunking effort to be restarted on next attempt for item ID "+
a,"error");k._maybeDeletePersistedChunkData(a);k.reevaluateChunking(a);k._getFileState(a).loaded=0},sendNext:function(a){var c=l.getSize(a),d=l.getName(a),e=m.nextPart(a),g=k._getChunkData(a,e),p=k._getFileState(a).attemptingResume,C=k._getFileState(a).chunking.inProgress||[];null==k._getFileState(a).loaded&&(k._getFileState(a).loaded=0);p&&!1===l.onResume(a,d,g)&&(m.reset(a),e=m.nextPart(a),g=k._getChunkData(a,e),p=!1);null==e&&0===C.length?m.finalize(a):(h(b.format("Sending chunked upload request for item {}.{}, bytes {}-{} of {}.",
a,e,g.start+1,g.end,c)),l.onUploadChunk(a,d,k._getChunkDataForCallback(g)),C.push(e),k._getFileState(a).chunking.inProgress=C,f&&n.open(a,e),f&&n.available()&&k._getFileState(a).chunking.remaining.length&&m.sendNext(a),0===g.blob.size?(h(b.format("Chunk {} for file {} will not be uploaded, zero sized chunk.",e,a),"error"),m.handleFailure(e,a,"File is no longer available",null)):k.uploadChunk(a,e,p).then(function(c,d){h("Chunked upload request succeeded for "+a+", chunk "+e);k.clearCachedChunk(a,e);
var g=k._getFileState(a).chunking.inProgress||[],f=q.normalizeResponse(c,!0),x=b.indexOf(g,e);h(b.format("Chunk {} for file {} uploaded successfully.",e,a));m.done(a,e,f,d);0<=x&&g.splice(x,1);k._maybePersistChunkedState(a);m.hasMoreParts(a)||0!==g.length?m.hasMoreParts(a)?m.sendNext(a):h(b.format("File ID {} has no more chunks to send and these chunk indexes are still marked as in-progress: {}",a,JSON.stringify(g))):m.finalize(a)},function(b,c){m.handleFailure(e,a,b,c)}).done(function(){k.clearXhr(a,
e)}))}},n={_open:[],_openChunks:{},_waiting:[],available:function(){var a=l.maxConnections,c=0,d=0;b.each(n._openChunks,function(a,b){c++;d+=b.length});return a-(n._open.length-c+d)},free:function(a,c){var d=!c,e=b.indexOf(n._waiting,a),g=b.indexOf(n._open,a);delete n._openChunks[a];q.getProxyOrBlob(a)instanceof b.BlobProxy&&(h("Generated blob upload has ended for "+a+", disposing generated blob."),delete k._getFileState(a).file);0<=e?n._waiting.splice(e,1):d&&0<=g&&(n._open.splice(g,1),d=n._waiting.shift(),
0<=d&&(n._open.push(d),q.start(d)))},getWaitingOrConnected:function(){var a=[];b.each(n._openChunks,function(b,c){c&&c.length&&a.push(parseInt(b))});b.each(n._open,function(b,c){n._openChunks[c]||a.push(parseInt(c))});return a=a.concat(n._waiting)},isUsingConnection:function(a){return 0<=b.indexOf(n._open,a)},open:function(a,b){null==b&&n._waiting.push(a);if(n.available()){if(null==b)n._waiting.pop(),n._open.push(a);else{var c=n._openChunks[a]||[];c.push(b);n._openChunks[a]=c}return!0}return!1},reset:function(){n._waiting=
[];n._open=[]}},p={send:function(a,b){k._getFileState(a).loaded=0;h("Sending simple upload request for "+a);k.uploadFile(a).then(function(c,d){h("Simple upload request succeeded for "+a);var e=q.normalizeResponse(c,!0),g=l.getSize(a);l.onProgress(a,b,g,g);q.maybeNewUuid(a,e);q.cleanup(a,e,d)},function(c,d){h("Simple upload request failed for "+a);var e=q.normalizeResponse(c,!1);l.onAutoRetry(a,b,e,d)||q.cleanup(a,e,d)})}},q={cancel:function(a){h("Cancelling "+a);l.paramsStore.remove(a);n.free(a)},
...
k._removeExpiredChunkingRecords()},isDeferredEligibleForUpload:function(a){return l.isQueued(a)},maybeDefer:function(a,c){if(c&&!k.getFile(a)&&c instanceof b.BlobProxy)l.onUploadPrep(a),h("Attempting to generate a blob on-demand for "+a),c.create().then(function(b){h("Generated an on-demand blob for "+a);k.updateBlob(a,b);l.setSize(a,b.size);k.reevaluateChunking(a);q.maybeSendDeferredFiles(a)},function(c){var d={};c&&(d.error=c);h(b.format("Failed to generate blob for ID {}.  Error message: {}.",
a,c),"error");l.onComplete(a,l.getName(a),b.extend(d,g),null);q.maybeSendDeferredFiles(a);n.free(a)});else return q.maybeSendDeferredFiles(a);return!1},maybeSendDeferredFiles:function(a){var c=l.getIdsInProxyGroup(a),d=!1;c&&c.length?(h("Maybe ready to upload proxy group file "+a),b.each(c,function(b,c){if(q.isDeferredEligibleForUpload(c)&&k.getFile(c))d=c===a,q.now(c);else if(q.isDeferredEligibleForUpload(c))return!1})):(d=!0,q.now(a));return d},maybeNewUuid:function(a,b){if(void 0!==b.newUuid)l.onUuidChanged(a,
b.newUuid)},normalizeResponse:function(a,c){var d=a;b.isObject(a)||(d={},b.isString(a)&&!c&&(d.error=a));d.success=c;return d},now:function(a){var d=l.getName(a);if(!c.isValid(a))throw new b.Error(a+" is not a valid file ID to upload!");l.onUpload(a,d);e&&k._shouldChunkThisFile(a)?m.sendNext(a):p.send(a,d)},start:function(a){var b=q.getProxyOrBlob(a);if(b)return q.maybeDefer(a,b);q.now(a);return!0}};b.extend(this,{add:function(a,b){k.add.apply(this,arguments)},upload:function(a){return n.open(a)?
q.start(a):!1},retry:function(a){f&&(k._getFileState(a).temp.ignoreFailure=!1);return n.isUsingConnection(a)?q.start(a):c.upload(a)},cancel:function(a){var c=k.cancel(a);b.isGenericPromise(c)?c.then(function(){q.cancel(a)}):!1!==c&&q.cancel(a)},cancelAll:function(){var a=n.getWaitingOrConnected(),b;if(a.length)for(b=a.length-1;0<=b;b--)c.cancel(a[b]);n.reset()},getFile:function(a){return k.getProxy&&k.getProxy(a)?k.getProxy(a).referenceBlob:k.getFile&&k.getFile(a)},isProxied:function(a){return!(!k.getProxy||
!k.getProxy(a))},getInput:function(a){if(k.getInput)return k.getInput(a)},reset:function(){h("Resetting upload handler");c.cancelAll();n.reset();k.reset()},expunge:function(a){if(c.isValid(a))return k.expunge(a)},isValid:function(a){return k.isValid(a)},getResumableFilesData:function(){return k.getResumableFilesData?k.getResumableFilesData():[]},getThirdPartyFileId:function(a){if(c.isValid(a))return k.getThirdPartyFileId(a)},pause:function(a){return c.isResumable(a)&&k.pause&&c.isValid(a)&&k.pause(a)?
(n.free(a),k.moveInProgressToRemaining(a),!0):!1},isResumable:function(a){return!!k.isResumable&&k.isResumable(a)}});b.extend(l,a);h=l.log;f=(e=l.chunking.enabled&&b.supportedFeatures.chunking)&&l.chunking.concurrent.enabled;g=function(){var a={};a[l.preventRetryParam]=!0;return a}();q.initHandler()};b.WindowReceiveMessage=function(a){var d={};b.extend({log:function(a,b){}},a);b.extend(this,{receiveMessage:function(a,e){var f=function(a){e(a.data)};window.postMessage?d[a]=b(window).attach("message",
f):log("iframe message passing not supported in this browser!","error")},stopReceivingMessages:function(a){window.postMessage&&(a=d[a])&&a()}})};b.FormUploadHandler=function(a){function d(a,c){var d=a.id,g=d.split("_")[0],f=p(g);h[f]=c;k[g]=b(a).attach("load",function(){e.getInput(g)&&(q("Received iframe load event for CORS upload request (iframe name "+d+")"),l[d]=setTimeout(function(){var a="No valid message received from loaded iframe for iframe name "+d;q(a,"error");c({error:a})},1E3))});r.receiveMessage(d,
function(a){q("Received the following window message: '"+a+"'");d.split("_");var b=e._parseJsonResponse(a),c=b.uuid;c&&h[c]?(q("Handling response for iframe name "+d),clearTimeout(l[d]),delete l[d],e._detachLoadEvent(d),a=h[c],delete h[c],r.stopReceivingMessages(d),a(b)):c||q("'"+a+"' does not contain a UUID - ignoring.")})}var c=a.options,e=this,f=a.proxy,g=b.getUniqueId(),h={},k={},l={},m=c.isCors,n=c.inputName,p=f.getUuid,q=f.log,r=new b.WindowReceiveMessage({log:q});b.extend(this,new b.UploadHandler(a));
...
_getChunkData:function(a,d){var e=g.partSize,f=n(a),h=c.getFile(a),k=e*d,e=k+e>=f?f:k+e,f=c._getTotalChunks(a),p=this._getFileState(a).temp.cachedChunks,h=p[d]||b.sliceBlob(h,k,e);p[d]=h;return{part:d,start:k,end:e,count:f,blob:h,size:e-k}},_getChunkDataForCallback:function(a){return{partIndex:a.part,startByte:a.start+1,endByte:a.end,totalParts:a.count}},_getLocalStorageId:function(a){var c=m(a),d=n(a),f=g.partSize;a=q(a);return b.format("qq{}resume{}-{}-{}-{}-{}",e,"5.0",c,d,f,a)},_getMimeType:function(a){return c.getFile(a).type},
...
a,m(a))),y(a,e.uuid),d.key=e.key,d.chunking=e.chunking,d.loaded=e.loaded,d.attemptingResume=!0,c.moveInProgressToRemaining(a)))},_maybePersistChunkedState:function(a){var d=c._getFileState(a),e;if(l&&c.isResumable(a)){e=c._getLocalStorageId(a);d={name:m(a),size:n(a),uuid:p(a),key:d.key,chunking:d.chunking,loaded:d.loaded,lastUpdated:Date.now()};try{localStorage.setItem(e,JSON.stringify(d))}catch(g){w(b.format("Unable to save resume data for '{}' due to error: '{}'.",a,g.toString()),"warn")}}},_registerProgressHandler:function(a,
d,e){var g=c._getXhr(a,d),f=m(a),h={simple:function(b,c){var d=n(a);b===c?v(a,f,d,d):v(a,f,b>=d?d-1:b,d)},chunked:function(g,h){var k=c._getFileState(a).temp.chunkProgress,p=c._getFileState(a).loaded,l=n(a),q=p;k[d]=g-(h-e);b.each(k,function(a,b){q+=b});v(a,f,q,l)}};g.upload.onprogress=function(a){if(a.lengthComputable)h[null==e?"simple":"chunked"](a.loaded,a.total)}},_registerXhr:function(a,b,d,e){b=null==b?-1:b;a=c._getFileState(a).temp;a.xhrs=a.xhrs||{};a.ajaxRequesters=a.ajaxRequesters||{};a.xhrs[b]=
...
log:c.log,onSend:c.onDelete,onComplete:c.onDeleteComplete,cors:c.cors}));b.extend(this,{sendDelete:function(a,b,g){g=g||{};c.log("Submitting delete file request for "+a);"DELETE"===c.method?d.initTransport(a).withPath(b).withParams(g).send():(g[c.uuidParamName]=b,d.initTransport(a).withParams(g).send())}})};(function(){function a(a,c,e,f){var m=document.createElement("canvas"),n=e.mime||"image/jpeg",p=new b.Promise;d(a,c,m,e,f).then(function(){p.success(m.toDataURL(n,e.quality||.8))});return p}function d(a,
d,f,l,m){var n=a.naturalWidth,p=a.naturalHeight,q=l.width,r=l.height,y=f.getContext("2d"),v=new b.Promise;y.save();if(l.resize)return c({blob:d,canvas:f,image:a,imageHeight:p,imageWidth:n,orientation:l.orientation,resize:l.resize,targetHeight:r,targetWidth:q});if(!b.supportedFeatures.unlimitedScaledImageSize){d=q;var w=r;if(!b.ios())throw new b.Error("Downsampled dimensions can only be reliably calculated for iOS!");d=5241E3<w*d?{newHeight:Math.round(Math.sqrt(w/d*5241E3)),newWidth:Math.round(Math.sqrt(d/
w*5241E3))}:void 0;d&&(b.log(b.format("Had to reduce dimensions due to device limitations from {}w / {}h to {}w / {}h",q,r,d.newWidth,d.newHeight),"warn"),q=d.newWidth,r=d.newHeight)}e(f,q,r,l.orientation);if(b.ios()){l=a.naturalWidth;d=a.naturalHeight;w=document.createElement("canvas");1048576<l*d?(w.width=w.height=1,d=w.getContext("2d"),d.drawImage(a,-l+1,0),l=0===d.getImageData(0,0,1,1).data[3]):l=!1;l&&(n/=2,p/=2);l=document.createElement("canvas");if(m){m=p;var x=document.createElement("canvas");
d=0;var t=w=m,C;x.width=1;x.height=m;x=x.getContext("2d");x.drawImage(a,0,0);for(x=x.getImageData(0,0,1,m).data;t>d;)C=x[4*(t-1)+3],0===C?w=t:d=t,t=w+d>>1;m=t/m;m=0===m?1:m}else m=1;q=Math.ceil(1024*q/n);r=Math.ceil(1024*r/p/m);d=m=0;l.width=l.height=1024;for(w=l.getContext("2d");m<p;){for(x=t=0;t<n;)w.clearRect(0,0,1024,1024),w.drawImage(a,-t,-m),y.drawImage(l,0,0,1024,1024,x,d,q,r),t+=1024,x+=q;m+=1024;d+=r}y.restore()}else y.drawImage(a,0,0,q,r);f.qqImageRendered&&f.qqImageRendered();v.success();
return v}function c(a){var c=a.blob,d=a.image,f=a.imageHeight,m=a.imageWidth,n=a.orientation,p=new b.Promise,q=a.resize,r=document.createElement("canvas"),y=r.getContext("2d"),v=a.canvas,w=a.targetHeight;a=a.targetWidth;e(r,m,f,n);v.height=w;v.width=a;y.drawImage(d,0,0);q({blob:c,height:w,image:d,sourceCanvas:r,targetCanvas:v,width:a}).then(function(){v.qqImageRendered&&v.qqImageRendered();p.success()},p.failure);return p}function e(a,b,c,d){switch(d){case 5:case 6:case 7:case 8:a.width=c;a.height=
...
e){e=e||{};var f=this,l=this.srcImage.naturalWidth,m=this.srcImage.naturalHeight,n=e.width,p=e.height,q=e.maxWidth,r=e.maxHeight,y=!this.blob||"image/jpeg"===this.blob.type,v=c.tagName.toLowerCase(),w;if(this.imageLoadListeners)this.imageLoadListeners.push(function(){f.render(c,e)});else if(n&&!p?p=m*n/l<<0:p&&!n?n=l*p/m<<0:(n=l,p=m),q&&n>q&&(n=q,p=m*n/l<<0),r&&p>r&&(p=r,n=l*p/m<<0),w={width:n,height:p},b.each(e,function(a,b){w[a]=b}),"img"===v?function(){var b=c.src;a(f.srcImage,f.blob,w,y).then(function(a){c.src=
a;b===c.src&&c.onload()})}():"canvas"===v&&d(this.srcImage,this.blob,c,w,y),"function"===typeof this.onrender)this.onrender(c)};b.MegaPixImage=f})();b.ImageGenerator=function(a){function d(a){return"img"===a.tagName.toLowerCase()}function c(a){return"canvas"===a.tagName.toLowerCase()}function e(a){a=a.split("/");a=a[a.length-1].split("?")[0];a=(a=b.getExtension(a))&&a.toLowerCase();switch(a){case "jpeg":case "jpg":return"image/jpeg";case "png":return"image/png";case "bmp":return"image/bmp";case "gif":return"image/gif";
...
10,2).then(function(a){d.success("4949"===a)});return d}function h(d,e){var f=new b.Promise;b.readBlobToHex(a,d+18,2).then(function(a){if(e)return f.success(c(a));f.success(parseInt(a,16))});return f}function k(a,d){var e=b.extend([],l),f={};b.each(d,function(b,d){var g=d.slice(0,4),h=a?c(g):parseInt(g,16),g=e.indexOf(h),k;0<=g&&(k=m[h].name,h=m[h].bytes,h=d.slice(16,16+2*h),f[k]=a?c(h):parseInt(h,16),e.splice(g,1));if(0===e.length)return!1});return f}var l=[274],m={274:{name:"Orientation",bytes:2}};
...
b.Scaler=function(a,d){var c=a.customResizer,e=a.sendOriginal,f=a.orient,g=a.defaultType,h=a.defaultQuality/100,k=a.failureText,l=a.includeExif,m=this._getSortedSizes(a.sizes);b.extend(this,{enabled:b.supportedFeatures.scaling&&0<m.length,getFileRecords:function(a,p,q){var r=this,y=[],v=q.blob?q.blob:q;(new b.Identify(v,d)).isPreviewableSync()?(b.each(m,function(a,e){var m=r._determineOutputType({defaultType:g,requestedType:e.type,refType:v.type});y.push({uuid:b.getUniqueId(),name:r._getName(p,{name:e.name,
type:m,refType:v.type}),blob:new b.BlobProxy(v,b.bind(r._generateScaledImage,r,{customResizeFunction:c,maxSize:e.maxSize,orient:f,type:m,quality:h,failedText:k,includeExif:l,log:d}))})}),y.push({uuid:a,name:p,size:v.size,blob:e?v:null})):y.push({uuid:a,name:p,size:v.size,blob:v});return y},handleNewFile:function(a,c,d,e,f,g,h,k){var l=[],m=null,H=k.addFileToHandler,E=k.uploadData,I=k.paramsStore,D=b.getUniqueId();b.each(this.getFileRecords(d,c,a),function(a,c){var d=c.size;c.blob instanceof b.BlobProxy&&
...
f=a.log,g=a.maxSize,h=a.orient,k=a.type,l=a.quality,m=a.failedText,n=a.includeExif&&"image/jpeg"===d.type&&"image/jpeg"===k,p=new b.Promise,q=new b.ImageGenerator(f),r=document.createElement("canvas");f("Attempting to generate scaled version for "+d.name);q.generate(d,r,{maxSize:g,orient:h,customResizeFunction:e}).then(function(){var a=r.toDataURL(k,l),e=function(){f("Success generating scaled version for "+d.name);var c=b.dataUriToBlob(a);p.success(c)};n?c._insertExifHeader(d,a,f).then(function(b){a=
b;e()},function(){f("Problem inserting EXIF header into scaled image.  Using scaled image w/out EXIF data.","error");e()}):e()},function(){f("Failed attempt to generate scaled version for "+d.name,"error");p.failure(m)});return p},_insertExifHeader:function(a,d,c){var e=new FileReader,f=new b.Promise,g="";e.onload=function(){g=e.result;f.success(b.ExifRestorer.restore(g,d))};e.onerror=function(){c("Problem reading "+a.name+" during attempt to transfer EXIF data to scaled version.","error");f.failure()};
...
g=this.KEY_STR.indexOf(a.charAt(h++)),b=b<<2|c>>4,c=(c&15)<<4|f>>2,e=(f&3)<<6|g,k.push(b),64!=f&&k.push(c),64!=g&&k.push(e);while(h<a.length);return k}}}();b.TotalProgress=function(a,d){var c={},e=0,f=0,g=-1,h=-1,k=function(b,c){b===g&&c===h||a(b,c);g=b;h=c},l=function(a,c){var d=!0;b.each(a,function(a,e){if(0<=b.indexOf(c,e))return d=!1});return d},m=function(a){var b=d(a);0<b&&(n(a,0,b),c[a]={loaded:0,total:b})},n=function(a,b,d){var g=c[a]?c[a].loaded:0;a=c[a]?c[a].total:0;-1===b&&-1===d?(e-=g,
f-=a):(b&&(e+=b-g),d&&(f+=d-a));k(e,f)};b.extend(this,{onAllComplete:function(a,b,c){if(0===b.length||l(b,c))k(f,f),this.reset()},onStatusChange:function(a,d,e){e===b.status.CANCELED||e===b.status.REJECTED?(n(a,-1,-1),delete c[a]):e===b.status.SUBMITTING&&m(a)},onIndividualProgress:function(a,b,d){n(a,b,d);c[a]={loaded:b,total:d}},onNewSize:function(a){m(a)},reset:function(){c={};f=e=0}})};b.PasteSupport=function(a){var d,c;d={targetElement:null,callbacks:{log:function(a,b){},pasteReceived:function(a){}}};
...
"submit"],g.type.toLowerCase());h&&!c(g)?d[g.name]=g.value:"select"!==g.tagName.toLowerCase()||c(g)||(h=e(g),null!==h&&(d[g.name]=h))});return d}});b.traditional=b.traditional||{};b.traditional.FormUploadHandler=function(a,d){function c(b,c){var d=a.paramsStore.get(b),h="get"===a.method.toLowerCase()?"GET":"POST",p=a.endpointStore.get(b),q=f(b);d[a.uuidName]=g(b);d[a.filenameParam]=q;return e._initFormForUpload({method:h,endpoint:p,params:d,paramsInBody:a.paramsInBody,targetName:c.name})}var e=this,
f=d.getName,g=d.getUuid,h=d.log;this.uploadFile=function(d){var f=e.getInput(d),g=e._createIframe(d),n=new b.Promise,p;p=c(d,g);p.appendChild(f);e._attachLoadEvent(g,function(c){h("iframe loaded");if(!c){var f,l,p;try{l=g.contentDocument||g.contentWindow.document,p=l.body.innerHTML,h("converting iframe's innerHTML to JSON"),h("innerHTML = "+p),p&&p.match(/^<pre/i)&&(p=l.body.firstChild.firstChild.nodeValue),f=e._parseJsonResponse(p)}catch(w){h("Error when attempting to parse form upload response ("+
w.message+")","error"),f={success:!1}}c=f}e._detachLoadEvent(d);a.cors.expected||b(g).remove();c.success?n.success(c):n.failure(c)});h("Sending upload request for "+d);p.submit();b(p).remove();return n};b.extend(this,new b.FormUploadHandler({options:{isCors:a.cors.expected,inputName:a.inputName},proxy:{onCancel:a.onCancel,getName:f,getUuid:g,log:h}}))};b.traditional=b.traditional||{};b.traditional.XhrUploadHandler=function(a,d){var c=this,e=d.getName,f=d.getSize,g=d.getUuid,h=d.log,k=a.forceMultipart||
...
h;d[a.totalFileSizeName]=k;d[a.chunking.paramNames.totalParts]=c._getTotalChunks(b);return d},p=function(a,c){var d={};try{h(b.format("Received response status {} with body: {}",c.status,c.responseText)),d=b.parseJson(c.responseText)}catch(e){a&&h("Error when attempting to parse xhr response text ("+e.message+")","error")}return d},q=function(d){var e=new b.Promise;l.complete(d,c._createXhr(d),n(d),a.customHeaders.get(d)).then(function(a){e.success(p(!1,a),a)},function(a){e.failure(p(!1,a),a)});return e},
r=function(c,d,h,l){var m=new FormData,n=a.method,p=a.endpointStore.get(l),q=e(l),r=f(l);c[a.uuidName]=g(l);c[a.filenameParam]=q;k&&(c[a.totalFileSizeName]=r);a.paramsInBody||(k||(c[a.inputName]=q),p=b.obj2url(c,p));d.open(n,p,!0);a.cors.expected&&a.cors.sendCredentials&&(d.withCredentials=!0);return k?(a.paramsInBody&&b.obj2FormData(c,m),m.append(a.inputName,h),m):h},y=function(d,e){var f=a.customHeaders.get(d),g=c.getFile(d);e.setRequestHeader("Accept","application/json");e.setRequestHeader("X-Requested-With",
"XMLHttpRequest");e.setRequestHeader("Cache-Control","no-cache");k||(e.setRequestHeader("Content-Type","application/octet-stream"),e.setRequestHeader("X-Mime-Type",g.type));b.each(f,function(a,b){e.setRequestHeader(a,b)})};b.extend(this,{uploadChunk:function(b,d,g){var h=c._getChunkData(b,d),l=c._createXhr(b,d);f(b);var n;n=m(b,l);c._registerProgressHandler(b,d,h.size);d=a.paramsStore.get(b);var p=f(b),q=e(b);d[a.chunking.paramNames.partIndex]=h.part;d[a.chunking.paramNames.partByteOffset]=h.start;
d[a.chunking.paramNames.chunkSize]=h.size;d[a.chunking.paramNames.totalParts]=h.count;d[a.totalFileSizeName]=p;k&&(d[a.filenameParam]=q);g&&(d[a.resume.paramNames.resuming]=!0);g=r(d,l,h.blob,b);y(b,l);l.send(g);return n},uploadFile:function(b){var d=c.getFile(b),e,f,g;f=c._createXhr(b);c._registerProgressHandler(b);e=m(b,f);g=a.paramsStore.get(b);d=r(g,f,d,b);y(b,f);f.send(d);return e}});b.extend(this,new b.XhrUploadHandler({options:b.extend({namespace:"traditional"},a),proxy:b.extend({getEndpoint:a.endpointStore.get},
d)}));b.override(this,function(c){return{finalizeChunks:function(d){return a.chunking.success.endpoint?q(d):c.finalizeChunks(d,b.bind(p,this,!0))}}})};b.traditional.AllChunksDoneAjaxRequester=function(a){var d,c={cors:{allowXdr:!1,expected:!1,sendCredentials:!1},endpoint:null,log:function(a,b){}},e={};b.extend(c,a);d=b.extend(this,new b.AjaxRequester({acceptHeader:"application/json",validMethods:["POST"],method:"POST",endpointStore:{get:function(a){return c.endpoint}},allowXRequestedWithAndCacheControl:!1,
...
d.showFilename(a),d.showEditIcon(a));g.addClass("qq-temp").removeClass("qq-temp")}}},_onUploadStatusChange:function(a,d,c){this._parent.prototype._onUploadStatusChange.apply(this,arguments);this._isEditFilenameEnabled()&&this._templating.getFileContainer(a)&&c!==b.status.SUBMITTED&&(this._templating.markFilenameEditable(a),this._templating.hideEditIcon(a));d===b.status.UPLOAD_RETRYING&&c===b.status.UPLOADING?(this._templating.hideRetry(a),this._templating.setStatusText(a),b(this._templating.getFileContainer(a)).removeClass(this._classes.retrying)):
...
!c.style.maxHeight&&b(c).css({maxWidth:d,maxHeight:e})},G=function(a,c){var d=n[a]||(new b.Promise).failure(),e=new b.Promise;p.then(function(a){d.then(function(){e.success()},function(){L(a,c);c.onload=function(){c.onload=null;e.success()};c.src=a.src;B(c)})});return e},K=function(a,c,d){var e=d.firstChild;0<c&&(e=b(d).children()[c].nextSibling);d.insertBefore(a,e)},O=function(a){var c=a.id,d=a.optFileOrBlob,e=d&&d.qqThumbnailId,g=F(c),h={customResizeFunction:a.customResizeFunction,maxSize:k,orient:!0,
...
b=l.placeholders.waitingForThumbnail,c={maxSize:k,scale:H};C&&(a?l.imageGenerator.generate(a,new Image,c).then(function(a){p.success(a)},function(){p.failure();r("Problem loading 'not available' placeholder image at "+a,"error")}):p.failure(),b?l.imageGenerator.generate(b,new Image,c).then(function(a){q.success(a)},function(){q.failure();r("Problem loading 'waiting for thumbnail' placeholder image at "+b,"error")}):q.failure())})();b.extend(this,{render:function(){r("Rendering template in DOM.");
f=0;x.innerHTML=w.template;A(u(x,m.dropProcessing));this.hideTotalProgress();t=l.fileContainerEl||u(x,m.list);r("Template rendering complete")},renderFailure:function(a){a=b.toElement(a);x.innerHTML="";x.appendChild(a)},reset:function(){this.render()},clearFiles:function(){t.innerHTML=""},disableCancel:function(){e=!0},addFile:function(a,f,g,h,k){var l=b.toElement(w.fileTemplate),n=u(l,m.file),p=u(x,m.uploader),r=k?d:t,v;k&&(c[a]=l);b(l).addClass("qq-file-id-"+a);p.removeAttribute("qq-drop-area-text");
n&&(b(n).setText(f),n.setAttribute("title",f));l.setAttribute("qq-file-id",a);g?K(l,g.index,r):r.appendChild(l);h?(l.style.display="none",b(l).addClass("qq-hidden-forever")):(A(J(a)),A(u(z(a),m.size)),A(u(z(a),m.deleteButton)),A(u(z(a),m.retry)),A(u(z(a),m.pause)),A(u(z(a),m.continueButton)),e&&this.hideCancel(a),(v=F(a))&&!v.src&&q.then(function(a){v.src=a.src;a.style.maxHeight&&a.style.maxWidth&&b(v).css({maxHeight:a.style.maxHeight,maxWidth:a.style.maxWidth});B(v)}))},addFileToCache:function(a,
