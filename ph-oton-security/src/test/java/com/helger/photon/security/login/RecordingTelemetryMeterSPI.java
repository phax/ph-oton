...
/**
 * An {@link ITelemetryMeterSPI} for the ph-oton-security tests that delegates every instrument to
 * the shared {@link CapturingTelemetry} in {@link #TELEMETRY}.<br>
 * It is registered via the regular SPI mechanism instead of {@link CapturingTelemetry#install()},
 * because the instruments of {@code LoginMetrics} and {@code LoginThrottleMetrics} are resolved
 * once in their static initializers. Other test classes of this module log users in and would
 * therefore bind those instruments to the no-op meter before a telemetry test could install
 * anything.
 *
