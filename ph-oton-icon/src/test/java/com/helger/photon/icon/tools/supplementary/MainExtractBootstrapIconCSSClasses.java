...
public class MainExtractBootstrapIconCSSClasses
{
  ...
  public static void main (final String [] args)
  {
        ...
        for (final ICSSSelectorMember aMember : aMembers)
          if (aMember instanceof CSSSelectorSimpleMember)
          {
            ...
            if (aSM.isClass ())
              ...
              {
                if (false)
                  System.out.println (aMembers.get (0));
                if (aMembers.get (0) instanceof CSSSelectorSimpleMember)
    ...
    for (final String sClass : aClassesIcon)
      if (sClass.startsWith (".bi-"))
      {
        final String sClassFieldName = createFieldName (sClass.substring (1));
        final String sFieldName = createFieldName (sClassFieldName.substring ("bi-".length ()));
        System.out.println (sFieldName + " (CBootstrapIconCSS." + sClassFieldName + "),");
      }
  }
}
